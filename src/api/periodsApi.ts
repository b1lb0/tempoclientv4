...
import { TimesheetApprovalPeriodsBean } from '../model/timesheetApprovalPeriodsBean';
...
export class PeriodsApi {
     ...
     */
    public async getPeriods (from: string, to: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TimesheetApprovalPeriodsBean;  }> {
        const localVarPath = this.basePath + '/periods';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: TimesheetApprovalPeriodsBean;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "TimesheetApprovalPeriodsBean");
