...
import { TeamLink } from '../model/teamLink';
import { TeamLinkInput } from '../model/teamLinkInput';
import { TeamLinkResults } from '../model/teamLinkResults';
...
export enum TeamLinksApiApiKeys {
}

export class TeamLinksApi {
    ...
    public setApiKey(key: TeamLinksApiApiKeys, value: string) {
        (this.authentications as any)[TeamLinksApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param teamLinkInput 
     */
    public async createTeamLink (teamLinkInput?: TeamLinkInput, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TeamLink;  }> {
        const localVarPath = this.basePath + '/team-links';
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(teamLinkInput, "TeamLinkInput")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: TeamLink;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "TeamLink");
     ...
     * @param linkId 
     */
    public async deleteTeamLink (linkId: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body?: any;  }> {
        const localVarPath = this.basePath + '/team-links/{linkId}'
     ...
     * @param linkId 
     */
    public async getTeamLink (linkId: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TeamLink;  }> {
        const localVarPath = this.basePath + '/team-links/{linkId}'
            .replace('{' + 'linkId' + '}', encodeURIComponent(String(linkId)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
     ...
     * @param id 
     */
    public async getTeamLinkByProjectId (id: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TeamLinkResults;  }> {
        const localVarPath = this.basePath + '/team-links/project/{id}'
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: TeamLinkResults;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "TeamLinkResults");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
