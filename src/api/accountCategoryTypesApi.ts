...
import { CategoryTypeResults } from '../model/categoryTypeResults';
...
export enum AccountCategoryTypesApiApiKeys {
}

export class AccountCategoryTypesApi {
    ...
    public setApiKey(key: AccountCategoryTypesApiApiKeys, value: string) {
        (this.authentications as any)[AccountCategoryTypesApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getTypes (options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CategoryTypeResults;  }> {
        const localVarPath = this.basePath + '/account-category-types';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CategoryTypeResults;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "CategoryTypeResults");
