...
export enum GlobalConfigurationsApiApiKeys {
}

export class GlobalConfigurationsApi {
    ...
    public setApiKey(key: GlobalConfigurationsApiApiKeys, value: string) {
        (this.authentications as any)[GlobalConfigurationsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getGlobalConfiguration (options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GlobalConfiguration;  }> {
        const localVarPath = this.basePath + '/globalconfiguration';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GlobalConfiguration;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GlobalConfiguration");
