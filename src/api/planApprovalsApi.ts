...
import { PlanApproval } from '../model/planApproval';
import { PlanApprovalInput } from '../model/planApprovalInput';
import { PlanApprovalSearchBean } from '../model/planApprovalSearchBean';
import { PlanApprovalsResult } from '../model/planApprovalsResult';
...
export enum PlanApprovalsApiApiKeys {
}

export class PlanApprovalsApi {
    ...
    public setApiKey(key: PlanApprovalsApiApiKeys, value: string) {
        (this.authentications as any)[PlanApprovalsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getPlansForReview (planApprovalSearchBean?: PlanApprovalSearchBean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PlanApprovalsResult;  }> {
        const localVarPath = this.basePath + '/plan-approvals/plans-for-review';
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(planApprovalSearchBean, "PlanApprovalSearchBean")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PlanApprovalsResult;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "PlanApprovalsResult");
     ...
     */
    public async getPossiblePlanReviewers (accountId: string, from?: string, to?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: string;  }> {
        const localVarPath = this.basePath + '/plan-permissions/plan-reviewers';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        // give precedence to 'application/json'
     ...
     */
    public async updatePlanApproval (allocationId: number, planApprovalInput?: PlanApprovalInput, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PlanApproval;  }> {
        const localVarPath = this.basePath + '/plan-approvals/{allocationId}'
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            useQuerystring: this._useQuerystring,
            json: true,
            body: ObjectSerializer.serialize(planApprovalInput, "PlanApprovalInput")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PlanApproval;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "PlanApproval");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
