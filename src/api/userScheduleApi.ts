...
import { DayScheduleResults } from '../model/dayScheduleResults';
...
export enum UserScheduleApiApiKeys {
}

export class UserScheduleApi {
    ...
    public setApiKey(key: UserScheduleApiApiKeys, value: string) {
        (this.authentications as any)[UserScheduleApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getAuthenticatedUserSchedule (from: string, to: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DayScheduleResults;  }> {
        const localVarPath = this.basePath + '/user-schedule';
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: DayScheduleResults;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "DayScheduleResults");
     ...
     * @param to The end date in format yyyy-mm-dd
     */
    public async getUserSchedule (accountId: string, from: string, to: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: DayScheduleResults;  }> {
        const localVarPath = this.basePath + '/user-schedule/{accountId}'
