...
import { CategoryInput } from '../model/categoryInput';
...
export enum AccountCategoriesApiApiKeys {
    Bearer = "Bearer"
}

export class AccountCategoriesApi {
    ...
    public setApiKey(key: AccountCategoriesApiApiKeys, value: string) {
        (this.authentications as any)[AccountCategoriesApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param categoryInput 
     */
    public async createCategory (categoryInput?: CategoryInput, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Category;  }> {
        const localVarPath = this.basePath + '/account-categories';
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(categoryInput, "CategoryInput")
        };
     ...
     * @param key 
     */
    public async deleteCategory (key: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body?: any;  }> {
        const localVarPath = this.basePath + '/account-categories/{key}'
     ...
     * @param id 
     */
    public async getCategories (id?: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CategoryResults;  }> {
        const localVarPath = this.basePath + '/account-categories';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CategoryResults;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "CategoryResults");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
     ...
     * @param key 
     */
    public async getCategoryByKey (key: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Category;  }> {
        const localVarPath = this.basePath + '/account-categories/{key}'
            .replace('{' + 'key' + '}', encodeURIComponent(String(key)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
     ...
     * @param categoryInput 
     */
    public async updateCategory (key: string, categoryInput?: CategoryInput, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Category;  }> {
        const localVarPath = this.basePath + '/account-categories/{key}'
            .replace('{' + 'key' + '}', encodeURIComponent(String(key)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
