...
import { SkillsAssignmentInput } from '../model/skillsAssignmentInput';
...
export enum SkillAssignmentsApiApiKeys {
}

export class SkillAssignmentsApi {
    ...
    public setApiKey(key: SkillAssignmentsApiApiKeys, value: string) {
        (this.authentications as any)[SkillAssignmentsApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async assignSkills (skillsAssignmentInput?: SkillsAssignmentInput, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<Skill>;  }> {
        const localVarPath = this.basePath + '/skill-assignments';
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(skillsAssignmentInput, "SkillsAssignmentInput")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Array<Skill>;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Array<Skill>");
     ...
     */
    public async getSkillAssignments (assigneeId: string, assigneeType: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<Skill>;  }> {
        const localVarPath = this.basePath + '/skill-assignments/{assigneeId}/{assigneeType}'
     ...
     */
    public async removeSkillAssignment (assigneeId: string, assigneeType: string, skillId: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body?: any;  }> {
        const localVarPath = this.basePath + '/skill-assignments/{assigneeId}/{assigneeType}/{skillId}'
            .replace('{' + 'assigneeId' + '}', encodeURIComponent(String(assigneeId)))
            .replace('{' + 'assigneeType' + '}', encodeURIComponent(String(assigneeType)))
     ...
     * @param skillsAssignmentInput 
     */
    public async replaceSkillAssignments (skillsAssignmentInput?: SkillsAssignmentInput, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<Skill>;  }> {
        const localVarPath = this.basePath + '/skill-assignments/replace';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        // give precedence to 'application/json'
